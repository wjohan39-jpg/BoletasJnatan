...
// try/catch blocks in the components that call db/storage/auth. Every
// caller already handles a null value gracefully (loading/error states, a
// failed login), so falling back to null here is safe.
function initFirebase() {
  ...
  try {
    ...
    return { db: getFirestore(app), storage: getStorage(app), auth: getAuth(app) };
  } catch {
    return { db: null, storage: null, auth: null };
  }
}

export const { db, storage, auth } = initFirebase();
